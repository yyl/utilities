...
- `convert_mp4_to_gif` ensures `output_path.parent` is created (`mkdir(parents=True, exist_ok=True)`) before encoding so batch conversions can target subdirectories without manual directory pre-creation.
